...
def DarknetConv2D_BN_Leaky_input(*args, **kwargs):
	...
	no_bias_kwargs = {'use_bias': False, 'input_shape': [416, 416, 3]}
...
def tiny_yolo_body_with_cfg(inputs, num_anchors, num_classes, cfg):
	"""
	Create Tiny YOLO_v3 model CNN body in keras.
	cfg corresponding shhould be
	             = ["conv2d_1",
					"conv2d_2",
					"conv2d_3",
					"conv2d_4",
	...
	x1 = compose(
			...
			MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[1], (3,3)),
			MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[2], (3,3)),
			MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[3], (3,3)),
			MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[4], (3,3)))(inputs)

	""" "conv2d_6", "conv2d_7", "conv2d_8" """
	x2 = compose(
			MaxPooling2D(pool_size=(2,2), strides=(2,2), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[5], (3,3)),
			MaxPooling2D(pool_size=(2,2), strides=(1,1), padding='same'),
			DarknetConv2D_BN_Leaky(cfg[6], (3,3)),
			DarknetConv2D_BN_Leaky(cfg[7], (1,1)))(x1)

	""" "conv2d_9", "conv2d_10" """
	y1 = compose(
			DarknetConv2D_BN_Leaky(cfg[9], (3,3)),
			DarknetConv2D(num_anchors*(num_classes+5), (1,1)))(x2)

	""" "conv2d_11" """
	x2 = compose(
			DarknetConv2D_BN_Leaky(cfg[8], (1,1)),
			UpSampling2D(2))(x2)
...
def yolo_eval_grad_am(yolo_outputs, yolo_model,
			  ...
			  score_threshold=.6,  # max_boxes=20, score_threshold=.6,iou_threshold=.5
			  iou_threshold=.5):
	"""Evaluate YOLO model on given input and return filtered boxes, and GradAM."""
	num_layers = len(yolo_outputs)
	anchor_mask = [[6,7,8], [3,4,5], [0,1,2]] if num_layers == 3 else [[3,4,5], [0,1,2]]  # default setting
	input_shape = K.shape(yolo_outputs[0])[1:3] * 32
	boxes = []
